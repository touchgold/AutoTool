 ...
 */
class DongFangTTHelper: BaseHelper() {
    ...
    companion object {
        const val TAG = "DongFangTTHelper"
        const val SCROLL_DELAY = 3 * 1000L
    }

    private val sListView = "com.songheng.eastnews:id/g9"
    private val sDetailRoot= "android.webkit.WebView"
    private var mMainRootNodeInfo:AccessibilityNodeInfo ?= null
...
//                            }
            ...
            if (mTargetNodeArray.size == 0) {
                ...
                if (mCount > 3) {
                    ...
                    //到达底部，刷新
                    AccessibilityUtil.clickByText(rootInActiveWindow, "新闻", true)
                    mRefresh = true
                    mCount = 0
                    return
                }
            ...
            while (mTargetNodeArray.size > 0 && mCurrentIndex < mTargetNodeArray.size) {
                ...
                if (mIsPaperTask) {
                    ...
                    mMainRootNodeInfo = rootInActiveWindow
//                    Toast.makeText(MyApplication.getApplication(), "进入详情页：${taskItem.text}", Toast.LENGTH_SHORT).show()
    ...
    private fun filterTargetNode(rootInActiveWindow: AccessibilityNodeInfo?, targetArray:MutableList<AccessibilityNodeInfo>) {
        var zhidingArray = mutableListOf<AccessibilityNodeInfo>()

        AccessibilityUtil.findTextArray(rootInActiveWindow, Regex("热门"), zhidingArray, true)
        ...
        for (zhiDingItem in zhidingArray) {
            ...
            for (i in 0 until targetArray.size) {
                var textRect = Rect()
                targetArray[i].getBoundsInScreen(textRect)
                if (abs(zhiDingRect.top - textRect.top) < 10) {
                    removeIndex = i
                }
            }
            if (removeIndex != -1) {
                Log.d(TAG, "readPaperTask remove　热门　" + targetArray[removeIndex].text)
                targetArray.removeAt(removeIndex)
            }
        }
